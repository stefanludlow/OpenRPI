...
void auction_notify(int id, int type) {
	MYSQL_RES *result = NULL;
	MYSQL_ROW row = NULL;
	OBJ_DATA *board = NULL;

	char buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH],
			buf3[MAX_STRING_LENGTH], buf4[MAX_STRING_LENGTH];
	*buf = '\0';
	*buf2 = '\0';
	*buf3 = '\0';
	*buf4 = '\0';
	...
	if (type == AUCTION_SALE) {
		...
		if ((row[13])) {
			...
			sprintf(buf3,
					"The item you had listed for sale with #5%s#0 has been sold.\n\n"
		...
		if ((board = vtoo(atoi(row[26])))) {
			...
			sprintf(buf, "#3Sale#0: #2%s#0 for #6%d#0 credits.", row[15],
					value);

			if (atoi(row[10]) > 0)
				sprintf(buf2, "and with a buyout set at #6%s#0 credits",
						row[10]);

			sprintf(buf3,
					"At this time, #2%s#0 was #3sold#0 for #6%d#0 credits to #5%s#0.\n\n"
...
void cancel_auction(CHAR_DATA *ch, CHAR_DATA *auctioneer, int id,
		bool confirmed) {
	MYSQL_RES *result = NULL;
	MYSQL_ROW row = NULL;
	char buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH],
	...
	*buf3 = '\0';
	...
	sprintf(buf, "SELECT * FROM %s.ah_auctions "
			...
			"AND port = %d AND placed_by = '%s'", world_log_db.c_str(), id,
			house_id, port, GET_NAME(ch));
	...
	if (!confirmed) {
		...
		sprintf(buf,
				"To be perfectly clear - you wish to cancel your auction of #2%s#0, and in doing so, lose "
						"the auction deposit that you placed with us?",
				row[15]);
	...
	sprintf(buf,
			"UPDATE %s.ah_auctions SET expires_at = UNIX_TIMESTAMP() WHERE auction_id = %d",
			world_log_db.c_str(), id);
	mysql_safe_query(buf);
	...
	name_to_ident(ch, buf2);
	sprintf(buf,
			"Very well. I have cancelled your auction for #2%s#0; we will be keeping the deposit for our trouble. "
					"You may #6RETRIEVE#0 your merchandise from me at your leisure.",
			row[15]);
	sprintf(buf3, "%s %s", buf2, buf);
	do_whisper(auctioneer, buf3, 83);

	sprintf(buf,
			"UPDATE %s.ah_auctions SET cancelled = 1 WHERE auction_id = %d",
			world_log_db.c_str(), id);
	mysql_safe_query(buf);
...
void preview_auction(CHAR_DATA *ch, CHAR_DATA *auctioneer, int id) {
	MYSQL_RES *result = NULL;
	OBJ_DATA *tobj = NULL;
	FILE *fp;
	char buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH],
	...
	*buf3 = '\0';
	...
	sprintf(buf,
			"SELECT * FROM %s.ah_auctions WHERE (expires_at > UNIX_TIMESTAMP()) AND auction_id = %d AND house_id = %d AND port = %d",
			world_log_db.c_str(), id, house_id, port);
	mysql_safe_query(buf);
	...
	if (!fp) {
		send_to_char(
				"There was an error loading this item from storage. Please contact staff.\n",
				ch);
		return;
	}
...
int retrieve_expiries(CHAR_DATA *ch, CHAR_DATA *auctioneer) {
	MYSQL_RES *result = NULL;
	MYSQL_ROW row = NULL;
	OBJ_DATA *tobj;
	FILE *fp;
	...
	*buf3 = '\0';
	...
	sprintf(buf,
			"SELECT * FROM %s.ah_returned_bids WHERE placed_by = '%s' AND picked_up = 0 AND port = %d ORDER BY amount ASC",
	...
	if ((row = mysql_fetch_row(result))) {
		name_to_ident(ch, buf2);
		sprintf(buf,
				"%s Better luck next time perhaps - I have %d credits here from an unsuccessful bid you placed some time ago. Here you are.",
				buf2, atoi(row[1]));
		do_whisper(auctioneer, buf, 83);
		...
		sprintf(buf,
				"UPDATE %s.ah_returned_bids SET picked_up = 1 WHERE placed_by = '%s' AND amount = %d",
				world_log_db.c_str(), GET_NAME(ch), atoi(row[1]));
		mysql_safe_query(buf);
	...
	sprintf(buf, "SELECT * FROM %s.ah_auctions"
			...
			" AND port = %d ORDER BY expires_at ASC", world_log_db.c_str(),
			GET_NAME(ch), port);
	mysql_safe_query(buf);
	...
	if ((row = mysql_fetch_row(result))) {
		...
		if (!fp) {
			send_to_char(
					"There was an error loading your item from storage. Please contact staff.\n",
					ch);
			return 1;
		}
		...
		name_to_ident(ch, buf2);
		sprintf(buf,
				"%s Here we are. As #2%s#0 did not sell, I am afraid we will need to keep the deposit fee. "
		...
		sprintf(buf,
				"UPDATE %s.ah_auctions SET seller_pickup = TRUE WHERE auction_id = %d",
		...
		if ((board = vtoo(atoi(row[26])))) {
			...
			sprintf(buf, "#3Fail#0: #2%s#0 for #6%d#0 credits.", row[15],
					value);
			...
			if (atoi(row[10]) > 0)
				sprintf(buf2, "and with a buyout set at #6%s#0 credits",
						row[10]);
			sprintf(buf3,
					"At this time, #2%s#0 #3failed to sell#0 for #6%d#0 credits.\n\n"
			...
			post_straight_to_mysql_board(fname((board)->name), buf,
					"AuctionHouse", p);
...
int retrieve_winnings(CHAR_DATA *ch, CHAR_DATA *auctioneer) {
	...
	sprintf(buf, "SELECT * FROM %s.ah_auctions"
			...
			" AND port = %d", world_log_db.c_str(), GET_NAME(ch), GET_NAME(ch),
			house_id, port);
	...
	if (!str_cmp(row[11], GET_NAME(ch)) && atoi(row[20]) == false) {
		...
		if (atoi(row[22]) <= 0)	// An auction won by a non-buyout, so we need to find the highest bidder.
				{
			sprintf(buf,
					"SELECT * FROM %s.ah_bids WHERE auction_id = %d ORDER BY bid_amount DESC LIMIT 1",
					world_log_db.c_str(), atoi(row[0]));
			...
			sprintf(buf,
					"UPDATE %s.ah_auctions SET sold_for = %d WHERE auction_id = %d",
					world_log_db.c_str(), sold_for, atoi(row[0]));
			mysql_safe_query(buf);
		} else
			sold_for = atoi(row[10]);
		...
		name_to_ident(ch, buf2);
		sprintf(buf,
				"%s Here we are. For the successful auction of #2%s#0, you will receive %d credits, plus a refund of your %d credit deposit, minus a House consignment "
						"fee of %d credits, for a net total of %d credits. Enjoy!",
				buf2, row[15], sold_for, atoi(row[5]), fee, net);
	...
	if (!str_cmp(row[9], GET_NAME(ch)) && atoi(row[19]) == false) {
		...
		name_to_ident(ch, buf2);
		sprintf(buf,
				"%s Ah, yes. I have something for you right here - #2%s#0, won at auction for a bargain "
						"price of %d credits. Congratulations again on your success!",
				buf2, row[15], sold_for);

		do_whisper(auctioneer, buf, 83);

		/* Let's set the auction record as picked up by the buyer so they can't dupe the item */
		sprintf(buf,
				"UPDATE %s.ah_auctions SET buyer_pickup = TRUE WHERE auction_id = %d",
				world_log_db.c_str(), atoi(row[0]));
		mysql_safe_query(buf);
...
void record_bid(CHAR_DATA *ch, CHAR_DATA *auctioneer, int id, int bid,
		bool bought_out, int buyout) {
	MYSQL_RES *result = NULL;
	MYSQL_ROW row = NULL;
	char buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH];
	...
	sprintf(buf,
			"SELECT bid_amount,placed_by FROM %s.ah_bids WHERE auction_id = %d AND port = %d ORDER BY bid_amount DESC LIMIT 1",
			world_log_db.c_str(), id, port);
	mysql_safe_query(buf);
	...
	if ((row = mysql_fetch_row(result))) {
		sprintf(buf,
				"INSERT INTO %s.ah_returned_bids (auction_id, amount, placed_by, house_id, port) VALUES "
						"(%d, %d, '%s', %d, %d)", world_log_db.c_str(), id,
				atoi(row[0]), row[1], house_id, port);
		mysql_safe_query(buf);
		if (result)
			mysql_free_result(result);
	}

	mysql_real_escape_string(database, buf2, char_short(ch),
			strlen(char_short(ch)));

	sprintf(buf,
			"INSERT INTO %s.ah_bids "
			...
			world_log_db.c_str(), id, bid, bought_out ? "TRUE" : "FALSE",
			GET_NAME(ch), buf2, ch->pc->account_name, port);
	...
	if (!bought_out) {
		...
		sprintf(buf, "UPDATE %s.ah_auctions "
				...
				" WHERE auction_id = %d", world_log_db.c_str(), new_bid,
				GET_NAME(ch), char_short(ch), id);
	} else {
...
void place_bid(CHAR_DATA *ch, CHAR_DATA *auctioneer, int id, int bid,
		bool confirmed) {
	MYSQL_RES *result = NULL;
	MYSQL_ROW row = NULL;
	char buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH],
	...
	*buf3 = '\0';
	...
	if (bid < atoi(row[8])) {
		name_to_ident(ch, buf2);
		sprintf(buf,
				"%s I am sorry, but the next available bid on this auction is set at a minimum of %s credits.",
				buf2, row[9]);
		do_whisper(auctioneer, buf, 83);
		if (result)
			mysql_free_result(result);
	...
	if (atoi(row[10]) && bid > atoi(row[10])) {
		name_to_ident(ch, buf2);
		sprintf(buf,
				"%s The buyout for this bid is only %s credits: there is no need to bid any higher.",
				buf2, row[10]);
		do_whisper(auctioneer, buf, 83);
		if (result)
			mysql_free_result(result);
	...
	if (!confirmed) {
		...
		sprintf(buf,
				"To make sure we understand one another - you wish to place a bid of %d credits upon #2%s#0. "
	...
	/* Record the bid and raise the next minimum bid on the item. */
		...
		if ((time_remaining - time(0)) < (60 * 60 * 6))	// Less than 24 IG hours/6 RL hours left on auction.
				{
			time_remaining = MIN((int) (time(0) + (60 * 60 * 6)),
					(time_remaining + (60 * 60)));

			sprintf(buf,
					"UPDATE %s.ah_auctions SET expires_at = %d WHERE auction_id = %d",
					world_log_db.c_str(), time_remaining, id);
			mysql_safe_query(buf);
		}
	}
...
void list_auctions(CHAR_DATA *ch, CHAR_DATA *auctioneer, char *argument,
		int cmd) {
	MYSQL_RES *result = NULL;
	MYSQL_ROW row = NULL;
	...
	if (cmd == -1)
		sprintf(buf, "SELECT * FROM %s.ah_auctions"
				...
				" ORDER BY expires_at ASC", world_log_db.c_str(), house_id,
				port, GET_NAME(ch), GET_NAME(ch));
	...
	*buf4 = '\0';
	...
	sprintf(buf + strlen(buf), " %7s##   %-35.35s   %-8.8s %-8.8s %-12.12s\n",
		...
		*statbuf = '\0';
		...
		if (!quicksell)
			sprintf(buf + strlen(buf),
					...
					row[0], row[15], buf2,
					atoi(row[10]) > atoi(row[9]) ? buf3 : "None", buf4);
		else
			sprintf(buf + strlen(buf),
					"%s%7s.   #2%-35.35s#0   %-8.8s %-8.8s %-12.12s\n", statbuf,
					row[0], row[15], "N/A",
...
void record_auction(CHAR_DATA *ch, CHAR_DATA *auctioneer, OBJ_DATA *obj,
	...
	*buf4 = '\0';
	...
	mysql_real_escape_string(database, buf2, obj->full_description,
			strlen(obj->full_description));
	mysql_real_escape_string(database, buf3, obj_desc(obj),
			strlen(obj_desc(obj)));
	mysql_real_escape_string(database, buf4, obj_short_desc(obj),
			strlen(obj_short_desc(obj)));
	...
	sprintf(buf,
			...
			world_log_db.c_str(), house_id, length, expires_at, deposit,
			real_value, min_bid, min_bid, buyout, GET_NAME(ch), char_short(ch),
			ch->pc->account_name, GET_ITEM_TYPE(obj), buf4, buf3, buf2,
			obj->nVirtual, port, char_short(auctioneer), auctioneer->circle);
...
void place_auction(CHAR_DATA *ch, CHAR_DATA *auctioneer, OBJ_DATA *obj,
	...
	*buf3 = '\0';
	...
	if (!auctioneer || !obj) {
		...
		error_message << "Error - " << __FILE__ << ": " << __func__ << " (line"
				<< __LINE__ << ")!\n";
	...
	if (buyout && buyout <= min_bid
			&& !(IS_SET(auctioneer->act, ACT_ECONZONE))) {
		name_to_ident(ch, buf2);
	...
	if (confirmed) {
		if (!can_subtract_money(ch, deposit, auctioneer->mob->currency_type)) {
			...
			record_auction(ch, auctioneer, obj, min_bid, buyout, length,
					deposit);
			extract_obj(obj);
	...
	if (IS_SET(auctioneer->act, ACT_ECONZONE))
		sprintf(buf,
				"Let's see if I have this right: you wish to auction #2%s#0 for a price of %d credits."
		...
		sprintf(buf,
				"Let's see if I have this right: you wish to auction #2%s#0 for a minimum bid of %d credits%s, "
				...
				obj->short_description, min_bid, buyout ? buf3 : "", length,
				deposit);
	...
	*buf2 = '\0';
	...
	if (!str_cmp(buf, "place")) {
		...
		sprintf(buf, "%s %s", buf2,
				"Unfortunately, the cancel auction feature is not working at present. Please contact an admin to have your item removed from auction.");
		do_whisper(auctioneer, buf, 83);
